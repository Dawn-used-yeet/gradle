...
class TestSuiteAutoReportingIntegrationTest extends AbstractIntegrationSpec {
    ...
    def "test task reports are generated for custom test tasks"() {
        ...
        then:
        result.assertTasksExecuted(":test", ":generateAggregateReport")

        when:
        succeeds("test2")

        then:
        result.assertTasksExecutedInOrder(":test2", ":generateAggregateReport")

        when:
        succeeds("test", "test2")

        then:
        result.assertTasksExecutedInOrder(":test", ":test2", ":generateAggregateReport")
        ...
        result.assertTaskNotExecuted(":generateAggregateReport")
    }
    ...
    def "test task reports are automatically generated for jvm test suites"() {
        ...
        then:
        result.assertTaskOrder(":test", ":generateAggregateReport")
        ...
        then:
        result.assertTaskOrder(":test2", ":generateAggregateReport")
        aggregateResults.totalNumberOfTestClassesExecuted == 1
        ...
        then:
        result.assertTaskOrder(":test", ":test2", ":generateAggregateReport")
    ...
    def "configuring tasks does not cause test aggregation task to execute"() {
        ...
        buildFile << """
            ...
            tasks.named("generateAggregateReport").configure {
    ...
    def "can run aggregate report without requesting tests as well"() {
        ...
        when:
        succeeds("generateAggregateReport")

        then:
        result.assertTasksExecuted(":generateAggregateReport")
